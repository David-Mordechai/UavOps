...
public class SimulatorInfraServiceTests
{
    ...
    private static SimulatorInfraService CreateSut(
        IVmwareController? vmwareController = null, ILessonLister? lessonLister = null, SimulatorOptions? options = null)
    {
        return new SimulatorInfraService(
            vmwareController ?? CreateReadyVmwareController(),
            lessonLister ?? Substitute.For<ILessonLister>(),
            options ?? new SimulatorOptions { LessonsFolder = @"C:\lessons" },
            NullLogger<SimulatorInfraService>.Instance);
    }
    ...
    [Fact]
    public async Task ListSimulatorLessons_ReturnsWhatTheListerLists()
    {
        var lister = Substitute.For<ILessonLister>();
        lister.ListLessons().Returns(new List<string> { "lesson1.ps1", "lesson2.ps1" });
        var sut = CreateSut(lessonLister: lister);
