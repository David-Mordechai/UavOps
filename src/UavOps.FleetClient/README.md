...
A .NET Framework 4.7 class library that connects to `UavOps.ControlApi`'s fleet command hub
(`/uavCommandHub`) as a SignalR client and relays every incoming command to an
...
3. That's it — `FleetClientConnection` registers a handler for every command, invokes your
   ...
   stall the connection), and replies to `UavOps.ControlApi` with the result.
...
- The DTOs here (`Waypoint`, `TelemetrySnapshot`, `GdtLinkStatus`, `MissionStatus`, `UavSummary`)
  are net47-side copies of `UavOps.ControlApi.Models` (no common TFM between net47 and net8 worth
  introducing). Property names must stay in sync by convention if the ControlApi side ever
  changes shape.
