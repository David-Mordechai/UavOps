...
/// <summary>
/// The uniform outcome of any reflected operation call — used by both the UAV domain
/// (<c>IOperationService</c>) and the simulator domain (<see cref="ISimulatorService"/>), which is
/// why this lives in the shared contracts project rather than either domain's own project.
/// Non-generic (<see cref="Value"/> is boxed) so <c>Tooling/OperationTool.cs</c> can invoke any
/// operation via plain reflection — no <c>dynamic</c>, no per-operation switch. Each concrete
/// implementation stays fully typed internally and only boxes to <see cref="object"/> at the
/// return statement.
/// </summary>
