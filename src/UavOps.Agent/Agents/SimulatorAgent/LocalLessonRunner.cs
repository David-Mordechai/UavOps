...
/// <summary>
/// Lists and runs <c>*.ps1</c> files from <see cref="SimulatorOptions.LessonsFolder"/>, a local
/// folder on this machine — no SSH/network hop, since the lessons live alongside the agent.
/// </summary>
public sealed class LocalLessonRunner(SimulatorOptions options) : ILocalLessonRunner
{
    ...
    public async Task<(int ExitCode, string Output, string Error)> RunLessonAsync(string lessonFileName, CancellationToken cancellationToken)
    {
        ...
        process.StartInfo.ArgumentList.Add(
            ...
            // instead) — forward $LASTEXITCODE explicitly so RunSimulatorLesson's success/failure
    ...
    /// ultimately originates from the operator's chat reply (via <c>AskOperatorChoiceTool</c>) —
    /// never trusted as a bare filename, since a reply like
    /// <c>"..\..\..\Windows\System32\evil.ps1"</c> must not be able to escape the lessons
    /// folder.</summary>
