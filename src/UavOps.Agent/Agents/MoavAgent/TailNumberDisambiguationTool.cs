...
/// <summary>
/// Wraps a <see cref="Tooling.OperationTool"/> for any fleet operation whose model-supplied
/// "tailNumber" argument might be a guess rather than something the operator actually said.
/// Domain-specific (lives here under <c>Agents/MoavAgent/</c>, not <c>Tooling/</c>) precisely
/// because it depends on <see cref="IOperationService.ListFleet"/>/<see cref="UavSummary"/> -
...
/// each tailNumber parameter's YAML description. This is deliberately model-side natural-language
...
/// see MoavAgent.yaml's instructions, which no longer try to detect "all"/"every" themselves).
///
...
public sealed class TailNumberDisambiguationTool : AIFunction
{
    ...
    public TailNumberDisambiguationTool(AIFunction inner, IOperationService operationService, OperatorPromptGate promptGate,
