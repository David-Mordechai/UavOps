...
/// Adapts the same request/response-over-SignalR mechanic <see cref="ChatHub"/>'s
/// <c>ConfirmationGate</c> uses for the browser chat client — send a message to the connected
/// client, wait on a correlated <see cref="TaskCompletionSource{TResult}"/> with a timeout —
/// adapted here for multiple concurrent in-flight operations targeting one specific connection,
/// rather than a single broadcast confirmation. All the waiting/matching logic lives in
