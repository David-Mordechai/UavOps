...
public sealed class RemoteOperationBroker(
    IHubContext<ChatHub, IOperationClientProxy> hub,
    ...
    TimeSpan? timeoutOverride = null) : IRemoteOperationBroker
{
    ...
    public void RegisterConnection(string connectionId)
    {
        lock (_connectionLock)
        {
            ...
            {
                logger.LogWarning(
                    "Moav command client {New} connected while {Previous} was still tracked — replacing it (last-writer-wins).",
                    connectionId, previous);
            }
            else
            {
                logger.LogInformation("Moav command client {ConnectionId} connected.", connectionId);
            }
    ...
    public void UnregisterConnection(string connectionId)
    {
        ...
        if (_pending.IsEmpty)
        {
            logger.LogInformation("Moav command client {ConnectionId} disconnected.", connectionId);
            return;
        }

        logger.LogWarning(
            "Moav command client {ConnectionId} disconnected — failing {Count} in-flight operation(s) immediately.",
        ...
        foreach (var correlationId in _pending.Keys.ToList())
        {
            ...
            {
                tcs.TrySetResult(new PendingReply(false, OperationError.NoClientConnected, "Moav command client disconnected.", null));
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (connectionId is null)
        {
            ...
            return OperationResult.Fail(OperationError.NoClientConnected, "No Moav command client is connected.");
        }
        ...
        catch (Exception ex)
        {
            ...
            logger.LogWarning(ex, "Failed to send operation (correlationId={CorrelationId}) to Moav command client.", correlationId);
            return OperationResult.Fail(OperationError.NoClientConnected, "Failed to reach the Moav command client.");
        }
        ...
        {
            logger.LogWarning(ex, "Moav command client returned malformed JSON for correlationId={CorrelationId}.", correlationId);
            return OperationResult.Fail(OperationError.ClientReportedError, "Moav command client returned an unreadable result.");
        }
    }

    private static string DefaultMessageFor(OperationError error) => error switch
    {
        OperationError.Timeout => "Moav command client did not respond in time.",
        OperationError.NoClientConnected => "No Moav command client is connected.",
        _ => "Moav command client reported an error."
    };
}
